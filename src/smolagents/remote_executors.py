...
class E2BExecutor(RemotePythonExecutor):
    ...
    def __init__(self, additional_imports: list[str], logger, **kwargs):
        ...
        try:
            ...
            raise ModuleNotFoundError("""Please install 'e2b' extra to use E2BExecutor: `pip install 'smolagents[e2b]'`""")
...
class DockerExecutor(RemotePythonExecutor):
    ...
    def __init__(
        ...
        try:
            ...
            raise ModuleNotFoundError("Please install 'docker' extra to use DockerExecutor: `pip install 'smolagents[docker]'`")
        self.host = host
        self.port = port
        ...
        try:
            ...
            if build_new_image:
                ...
                _, build_logs = self.client.images.build(path=str(dockerfile_path.parent), dockerfile=str(dockerfile_path), tag=self.image_name)
            ...
            self.installed_packages = self.install_packages(additional_imports)
            self.logger.log(f"Container {self.container.short_id} is running with kernel {self.kernel_id}", level=LogLevel.INFO)
    ...
    def _send_execute_request(self, code: str) -> str:
        ...
        execute_request = {
            "header": {"msg_id": msg_id, "username": "anonymous", "session": str(uuid.uuid4()), "msg_type": "execute_request", "version": "5.0",},
            "parent_header": {},
            "metadata": {},
