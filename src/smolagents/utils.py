...
def _is_package_available(package_name: str) -> bool:
    try:
        ...
        return False
BASE_BUILTIN_MODULES = ["collections", "datetime", "itertools", "math", "queue", "random", "re", "stat", "statistics", "time", "unicodedata",]
...
def parse_code_blobs(text: str) -> str:
    ...
    if "final" in text and "answer" in text:
        raise ValueError(dedent(f"""Your code snippet is invalid, because the regex pattern ```(?:py|python)?\\s*\\n(.*?)\\n``` was not found in it. Here is your code snippet: {text} It seems like you're trying to return the final answer, you can do it as follows: Code: ```py final_answer("YOUR FINAL ANSWER HERE")``` <end_code> """).strip())
    raise ValueError(
        dedent(
            f"""
...
def instance_to_source(instance, base_cls=None):
    ...
    class_attrs = {name: value for name, value in cls.__dict__.items() if not name.startswith("__") and not callable(value) and not (base_cls and hasattr(base_cls, name) and getattr(base_cls, name) == value)}
    ...
    if class_attrs:
        class_lines.append("")
    methods = {name: func for name, func in cls.__dict__.items() if callable(func) and (not base_cls or not hasattr(base_cls, name) or (isinstance(func, staticmethod) or isinstance(func, classmethod) or (getattr(base_cls, name).__code__.co_code != func.__code__.co_code)))}
